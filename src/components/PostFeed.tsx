...
const PostFeed = ({ posts, highlightQuery }: PostFeedProps) => {
  ...
  const handleSubmitComment = () => {
    if (newComment.trim() && selectedPost) {
      ...
      // Update context for real-time sync across components
      addComment(selectedPost.id, newCommentObj);
  ...
  const confirmDeleteComment = () => {
    if (commentToDelete) {
      if (commentToDelete.hasReplies) {
        // Mark comment as deleted but keep replies
      ...
      if (selectedPost) {
        const remainingStaticComments = (selectedPost.commentsData || []).filter(c => c.id !== commentToDelete.id);
        const remainingDynamicComments = (dynamicComments[selectedPost.id] || []).filter(c => c.id !== commentToDelete.id);
        const totalRemaining = remainingStaticComments.length + remainingDynamicComments.length;
        
        if (totalRemaining === 0 && (selectedPost.isDeleted || deletedPosts.has(selectedPost.id))) {
          // Close modal if post is deleted and no comments left
          handleCloseModal();
        } else if (!commentToDelete.hasReplies) {
          // Update selected post to reflect removed comment
          if (selectedPost.commentsData) {
            selectedPost.commentsData = selectedPost.commentsData.filter(c => c.id !== commentToDelete.id);
          }
        }
      }
  ...
  const confirmDeleteReply = () => {
    if (replyToDelete && selectedPost) {
      setDynamicComments(prev => {
        ...
        // Find and update the comment that contains this reply
        const commentIndex = postComments.findIndex(c => c.id === replyToDelete.commentId);
        if (commentIndex !== -1) {
          postComments[commentIndex] = {
            ...postComments[commentIndex],
            replies: postComments[commentIndex].replies.filter((r: any) => r.id !== replyToDelete.id)
          };
          updatedComments[selectedPost.id] = postComments;
        } else {
          // It's from static data, need to handle differently
          if (selectedPost.commentsData) {
            const staticComment = selectedPost.commentsData.find(c => c.id === replyToDelete.commentId);
            if (staticComment && staticComment.replies) {
              staticComment.replies = staticComment.replies.filter((r: any) => r.id !== replyToDelete.id);
            }
          }
        }
      ...
      // Also remove from static data if needed
      if (selectedPost.commentsData) {
        const commentIndex = selectedPost.commentsData.findIndex(c => c.id === replyToDelete.commentId);
        if (commentIndex !== -1) {
          const comment = selectedPost.commentsData[commentIndex];
          if (comment.replies) {
            const replyIndex = comment.replies.findIndex((r: any) => r.id === replyToDelete.id);
            if (replyIndex !== -1) {
              comment.replies.splice(replyIndex, 1);
              
              // Check if comment should be removed (if it was deleted and has no more replies)
              if (deletedComments.has(comment.id) && comment.replies.length === 0) {
                selectedPost.commentsData.splice(commentIndex, 1);
                setDeletedComments(prev => {
                  const newSet = new Set(prev);
                ...
                if ((selectedPost.isDeleted || deletedPosts.has(selectedPost.id)) && 
                    selectedPost.commentsData.length === 0 && 
                    (dynamicComments[selectedPost.id]?.length || 0) === 0) {
                  handleCloseModal();
                }
              }
            }
  ...
  const renderTextWithMentions = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith('@')) {
        return (
          <strong key={index} style={{ color: 'var(--color-black)' }}>
            {part}
          </strong>
        );
      }
  ...
  const handleSubmitReply = () => {
    if (replyText.trim() && replyingTo && selectedPost) {
      ...
      setDynamicComments(prev => {
        ...
        const commentIndex = postComments.findIndex(c => c.id === replyingTo.commentId);
        if (commentIndex !== -1) {
          // Update dynamic comment
          postComments[commentIndex] = {
            ...postComments[commentIndex],
            replies: [...(postComments[commentIndex].replies || []), newReply]
          };
          updatedComments[selectedPost.id] = postComments;
        } else {
          // It's a static comment, need to handle differently
          if (selectedPost.commentsData) {
            const staticComment = selectedPost.commentsData.find(c => c.id === replyingTo.commentId);
            if (staticComment) {
              if (!staticComment.replies) {
                staticComment.replies = [];
              }
              staticComment.replies.push(newReply);
            }
          }
        }
  ...
  const confirmDeletePost = () => {
    if (postToDelete) {
      ...
      if (hasComments) {
        // Mark post as deleted but keep it visible with comments
